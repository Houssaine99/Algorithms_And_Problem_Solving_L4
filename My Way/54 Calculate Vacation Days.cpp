...
int VacationPeriod(stDate From, stDate To)
{
	int Days = 0;
	short DayOrder = 0;
	while (isDate1LessThanDate2(From, To))
	{
		DayOrder = DayOfWeekOrder(From);
...
int main()
{
	...
	std::cout << "Vacation End:\n";
	EndDate = ReadFullDate();

	std::cout << "Vacation From: " << DayShortName(DayOfWeekOrder(StartDate))
		<< ", " << StartDate.Day << "/" << StartDate.Month << "/" << StartDate.Year << std::endl;

	std::cout << "Vacation To: " << DayShortName(DayOfWeekOrder(EndDate))
		<< ", " << EndDate.Day << "/" << EndDate.Month << "/" << EndDate.Year << std::endl;

	std::cout << "\nActual Vacation Days is: " << VacationPeriod(StartDate, EndDate) << std::endl;
